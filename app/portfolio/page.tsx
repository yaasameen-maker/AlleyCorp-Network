...
async function getPortfolioData(): Promise<CompanyGroup[]> {
  try {
    ...
    const companyMap = new Map<string, CompanyGroup>(
      allCompanies.map((pc) => [
        pc.id,
        { id: pc.id, name: pc.name, website: pc.website ?? "", status: pc.status ?? "active", coInvestors: [] },
      ])
    );
        ...
        <div className="max-w-5xl mx-auto px-8 py-5 flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-xs text-[#9CA3AF] mt-0.5">
              {activeCount} active · {alumniCount} alumni · {withCoInvestors.length} with co-investors · {totalLinks} co-investor links tracked
            </p>
          </div>
        </div>
      </header>
            ...
            >
              ...
              <div className="px-5 py-3.5 flex items-center justify-between">
                <div className="flex items-center gap-2.5 min-w-0">
                  <p className={`text-sm font-bold truncate ${isAlumni ? "text-[#9CA3AF]" : "text-[#0D1320]"}`}>
                    {company.name}
                  </p>
                  {isAlumni && (
              ...
              {company.coInvestors.length > 0 && (
                <>
                  <div className="grid grid-cols-[1fr_auto_100px] px-5 py-2 gap-6 bg-[#FAFAFA] border-t border-[#F3F4F6]">
                    <span className="text-[9px] uppercase tracking-widest text-[#C4C9D4] font-semibold">Fund</span>
                    <span className="text-[9px] uppercase tracking-widest text-[#C4C9D4] font-semibold">Warmth</span>
                    <span className="text-[9px] uppercase tracking-widest text-[#C4C9D4] font-semibold">Last signal</span>
                  </div>
